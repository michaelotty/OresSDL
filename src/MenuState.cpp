...
MenuState::MenuState(Game* game, std::shared_ptr<RenderWrapperBase> render)
    : game(game), render(render)
{
    menuRenderer = std::make_shared<MenuRenderer>(render);

    Button playButton = {PLAY_BUTTON_X,
                         WINDOW_HEIGHT / 2,
                         PLAY_BUTTON_WIDTH,
                         WINDOW_HEIGHT / 10,
                         "Play",
...
void MenuState::Update(float deltaTime) { menuRenderer->UpdateRender(buttons); }
...
void MenuState::OnPlayButtonClick() { game->SwitchState(game->inGameState); }
