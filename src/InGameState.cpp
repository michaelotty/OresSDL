...
InGameState::InGameState(Game* game, std::shared_ptr<RenderWrapperBase> render)
{
    ...
    Button pushButton = {WINDOW_WIDTH / 1.1f,
                         WINDOW_HEIGHT - BOARD_INITIALCOLUMN_HEIGHT_POS +
                             WINDOW_HEIGHT / 40,
                         WINDOW_WIDTH / 20,
                         WINDOW_HEIGHT / 20,
                         "<push",
...
void InGameState::Update(float deltaTime)
{
    ...
    gameRenderer->UpdateRender(buttons, score, levelScore, currentLevel,
                               pointsToNextLevel, pushTimer, PUSH_TIMER);
}
...
void InGameState::OnMouseLeftClick(int PosX, int PosY)
{
    ...
    if (CanGoToNextLevel)
    {
        ...
        currentLevel++;
        pointsToNextLevel =
            std::ceil(pointsToNextLevel * NEXT_LEVEL_SCORE_MULTIPLY);
        boardLogic->ResetBoard();
    }
