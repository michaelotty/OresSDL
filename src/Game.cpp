...
{
    last_frame_time = 0;
    render = std::make_shared<SDLRenderWrapper>(
        WINDOW_TITLE, WINDOW_WIDTH, WINDOW_HEIGHT, WINDOW_FULLSCREEN);
...
void Game::Update()
{
    ...
    float deltaTime = (currentTicks - last_frame_time) /
                      1000.0f; // Convert milliseconds to seconds
    ...
    while (
        !SDL_TICKS_PASSED(SDL_GetTicks(), last_frame_time + FRAME_TARGET_TIME))
        ;
}

void Game::OnQuitWindowClick() { Running = false; }
