...
SDLRenderWrapper::~SDLRenderWrapper() { TTF_Quit(); }

void SDLRenderWrapper::UpdateRender() { SDL_RenderPresent(renderer); }
...
bool SDLRenderWrapper::LoadTexture(std::string fileName)
{
    ...
    if (tempSurf == 0)
    {
        std::cout << "could not load image named: '" << fileName.c_str()
                  << "'!!!" << std::endl;
        return false;
    }
    ...
    if (texture != 0)
    {
        std::cout << "Texture loaded successfully... ('" << fileName.c_str()
                  << "')" << std::endl;
...
void SDLRenderWrapper::RenderImage(std::string filename, int x, int y, int w,
                                   int h, double scale,
                                   std::optional<Color> color)
{
    ...
    if (color.has_value())
    {
        SDL_SetTextureColorMod(textureMap[filename], color->red, color->green,
                               color->blue);
    }

    if (SDL_RenderCopyEx(renderer, textureMap[filename], NULL, &destRect, 0,
                         NULL, flip) != 0)
...
void SDLRenderWrapper::ClearAllTextures() { textureMap.clear(); }

void SDLRenderWrapper::RenderText(const std::string& text,
                                  const std::string& fontFile, int fontSize,
                                  int x, int y, double scale, bool centered,
                                  std::optional<Color> color)
{
    // Load font
...
void SDLRenderWrapper::DrawRectangle(int x, int y, float width, float height,
                                     Color color)
{
    // Set render color
